...
bool Minimiser::initialiseContinueVariables(){
  bool dbThis=false;
  if(! parsOK()) return false;
  
  bool success=true;
  if(dbThis) {
    cout << "Minimiser::initialiseContinueVariables() called. " << endl;
    cout << "\n\t(declaring them to MINUIT)" << endl;
  } 
...
bool Minimiser::CallMigrad(){ 
  bool dbThis=true;
  bool success=true;
  arglist[0] = _maxCalls; arglist[1] = _tollerence;
  if(dbThis) cout << "calling MIGRAD" << endl;
  TMinuit::mnexcm("MIGRAD", arglist ,2,ierflg);
  TMinuit::mnexcm("HES"   , arglist ,1,ierflg);
...
bool Minimiser::prepContinueFit(){
  bool dbThis=false;
  bool success=true;
  success &= SetSomeMinuitOptions();
  theFunction()->beginFit();
  success &= this->initialiseContinueVariables();
  if(dbThis) cout << "re-initialised variables" << endl;
  return success;
}
...
bool Minimiser::continueFit(){
  bool dbThis=false;
  bool success = true;
  if(dbThis) cout << "Minimiser::continueFit() called" << endl;
  success &= prepContinueFit();
  if(dbThis) cout << "... called prepFit " << success << endl;
  success &= CallMigrad();
  if(dbThis) cout << "... called MIGRAD" << success << endl;
  scanMarked();
...
bool Minimiser::continueMinosFit(){
  bool dbThis=false;
  bool success = true;
  success &= prepContinueFit();
  success &= CallMinos();
...
void Minimiser::setStrategy(int val){
...
void Minimiser::setTollerance(double val){
